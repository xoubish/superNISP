...
def validate_model(model, val_loader, criterion, device, config):
    """Validation loop."""
    model.eval()
    val_loss = 0.0
    val_shape_errors = []
    ...
    with torch.no_grad():
        for lr_batch, hr_batch in val_loader:
            ...
            with torch.amp.autocast(device_type='cuda' if device.type == 'cuda' else 'cpu'):
                ...
                # Loss is between predicted and actual noise
...
def main():
    ...
    for epoch in range(config.epochs):
        ...
        # Validation
        val_loss, val_shape_error = validate_model(model, val_loader, criterion, device, config)
